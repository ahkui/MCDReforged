...
def on_load(server, old):
    server.add_help_message('!!gm', '切換玩家模式')
    server.add_help_message('!!gm [ID]', '管理員強制切換玩家模式(限制權限admin)')
  ...
  if info.content.startswith('!!gm') and info.is_player == 1:
    if per >= 1:
      ...
      if len(gm) == 2 and per == 3:
          server.tell(info.player, '即將更改玩家 §6{0} §r的 §a遊戲模式!'.format(gm[1]))
          if os.path.exists('./plugins/gm/' + gm[1]):
            server.tell(gm[1], '§6正在更改模式，請不要走動!')
            time.sleep(1)
            ...
            server.tell(gm[1], '§6已切換到觀察者模式,要切換回来請輸入!!gm')
      elif len(info.content) != 4:
        server.tell(info.player, '§6裝你媽的B')
      elif os.path.exists('./plugins/gm/' + info.player):
        server.tell(info.player, '§6正在更改模式，請不要走動!')
        time.sleep(1)
        ...
        server.tell(info.player, '§6已切換到觀察者模式,要切換回来請輸入!!gm')
    else:
      server.tell(info.player, '你没有權限')
